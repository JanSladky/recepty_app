// 📁 backend/src/utils/parseNested.ts
export type FlatFields = Record<string, string | undefined>;
 ...
 */
export function parseIngredients(fields: FlatFields) {
  const idxMap = new Map<number, Record<string, any>>();
  ...
  for (const [k, v] of Object.entries(fields)) {
    ...
    if (!idxMap.has(index)) idxMap.set(index, {});
