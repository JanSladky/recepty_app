...
export async function updateRecipeInDB(
  ...
  try {
    ...
    // ✅ Buď upravíme i obrázek, nebo ho zachováme původní
    if (imageUrl !== null) {
      await client.query("UPDATE recipes SET title = $1, description = $2, image_url = $3 WHERE id = $4", [title, description, imageUrl, id]);
    } else {
      await client.query("UPDATE recipes SET title = $1, description = $2 WHERE id = $3", [title, description, id]);
    }

    // ❌ smažeme staré relace
